...
const Login = () => {
    ...
    const handleLogin = async () => {
        try {
            const response = await fetch('https://azhzx0jphc.execute-api.eu-north-1.amazonaws.com/dev/auth/login-email', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    username: email, // Ensure email is passed as the username in the login request
                    password,
                }),
            });
